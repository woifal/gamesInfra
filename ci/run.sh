...
containercmd run -d -it --name gamesbackend -e dbPassword_prod=${dbPassword_prod}  -p 9005:9000 woifal/gamesbackend:1.6
#docker exec -it gamesbackend /bin/bash

containercmd container rm -f gamesbackenddev
#docker run -d -it --name nodeplusora -p 9001:9000 --entrypoint /bin/bash nodeplusora:0.1
containercmd run -d -it --name gamesbackenddev -e dbPassword_dev=${dbPassword_dev}  -p 9005:9000 woifal/gamesbackend_dev:1.6
#docker exec -it gamesbackend /bin/bash
...
containercmd run -d -it --name gamesapp -e PORT=80 -p 80:80 -dt woifal/gamesapp:1.6
#docker exec -it gamesbackend /bin/bash
